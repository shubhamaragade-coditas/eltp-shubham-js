...
const items = [2, 3, 'ram', 55, 'Pune', false]
const getStringLists = (items) => {
    return items.filter((item) => typeof item === 'string')
}

console.log(getStringLists(items))

// Use reduce to sum all the numbers in the numbers array.
console.log(numbers.reduce((accumulator, currentValue) => accumulator + currentValue));

// Use reduce to concatenate all the countries and to produce this sentence: Estonia, Finland, Sweden, Denmark, Norway, and IceLand are north European countries
console.log(['Estonia', 'Finland', 'Sweden', 'Denmark', 'Norway', 'IceLand'].reduce((accumulator, currentValue) => accumulator + currentValue + ' ') + 'are north European countries');

// Explain the difference between some and every
// The some method checks if at least one element in the array satisfies the given condition. It returns true if there is at least one element that passes the condition, and false otherwise. It stops iterating through the array as soon as it finds the first element that satisfies the condition. If no element satisfies the condition, it will still iterate through the entire array before returning false
...
// The find method returns the value of the first element in the array that satisfies the provided condition.  If no element satisfies the condition, it returns undefined.
// the findIndex method returns the index of the first element in the array that satisfies the provided condition.  If no element satisfies the condition, it returns -1
...
console.log(products.filter(item => typeof item.price === 'number')
    .map(item => item.price)
...
const countriesByPopulation = countries.slice().sort((a, b) => a.population - b.population);
...
function mostSpokenLanguages(countries, noOfCountries) {
  ...
  const languageData = Object.entries(languageCounts).map(([language, count]) => ({ country: language, count }));

  return languageData.sort((a, b) => b.count - a.count).slice(0, noOfCountries);
}
